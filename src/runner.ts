...
const MEMORY_MD = join(process.cwd(), "memory/MEMORY.md");
...
export async function runCompact(
  ...
  const result = await runClaudeOnce(compactArgs, model, baseEnv, timeoutMs);
...
async function execClaude(name: string, prompt: string): Promise<RunResult> {
  ...
  if (existsSync(IDENTITY_MD)) {
    ...
    try {
      const memoryMd = await Bun.file(MEMORY_MD).text();
      if (memoryMd.trim()) appendParts.push(memoryMd.trim());
    } catch (e) {
      if (!isNotFoundError(e)) {
        console.error(`[${new Date().toLocaleTimeString()}] Failed to read MEMORY.md:`, e);
      }
    }
  }
  ...
  const exec = await runClaudeOnce(args, model, baseEnv, timeoutMs);
  ...
  if (COMPACT_TIMEOUT_ENABLED && exitCode === 124 && !isNew && existing) {
    ...
    if (compactOk) {
      ...
      const retryExec = await runClaudeOnce(args, model, baseEnv, timeoutMs);
